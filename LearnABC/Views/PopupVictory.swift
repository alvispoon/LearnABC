...
class PopupVictory: UIView {
  ...
  required init(coder aDecoder:NSCoder) {
    ...
    init(frame:CGRect, words: NSArray, score: Int) {
    ...
    print (levelTitleLabel.frame)
        ...
        let victoryIV = UIImageView(image: trophyImage)
        let scale = frame.width*0.6 / trophyImage.size.width
       
        victoryIV.frame = CGRect(x: frame.width*0.15, y: frame.height*0.25 , width: trophyImage.size.width * scale, height:
        trophyImage.size.height * scale)
        victoryIV.center = CGPoint(x: frame.width*0.5, y: frame.height*0.5)
        self.addSubview(victoryIV)
        
        let scoreLabel = UILabel(frame: CGRect(x: 0, y: 0, width: trophyImage.size.width*0.75, height: trophyImage.size.width*0.75))
        scoreLabel.textAlignment = .center
        scoreLabel.center = CGPoint(x:frame.width/2, y:frame.height*0.4)
        scoreLabel.backgroundColor =  UIColor.clear
        scoreLabel.text = "\(score)"
        scoreLabel.font = UIFont(name:"Arial-BoldMT" , size: 70.0)!
        scoreLabel.textColor = .black
        scoreLabel.textAlignment = .center
        
                        scoreLabel.minimumScaleFactor = 0.1    //or whatever suits your need
...
//
        ...
        BackButton.frame = CGRect(x: frame.width*0.4 , y: frame.height*0.8, width: buttonwidth, height: buttonwidth)
